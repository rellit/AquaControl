...
import { addSwitchTime, changeRelays, changeSwitchTime, deleteSwitchTime, loadedRelayState, loadedSettings, loadSettings, loadSettingsError, saveSettings, switchRelay } from './aqua.actions';
...
export const aquaReducer = createReducer(
    ...
    on(changeSwitchTime, (state, payLoad) => { return { ...state, status: { status: 'unsaved', errorMessage: state.status.errorMessage }, times: state.times.map(t=>t.id===payLoad.switchTime.id?payLoad.switchTime:t) } }),
    on(changeRelays, (state, payLoad) => { console.log(payLoad); return { ...state, status: { status: 'unsaved', errorMessage: state.status.errorMessage }, relays: payLoad.relays } }),
);
